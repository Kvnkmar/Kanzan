...
> - **`C_FORCE_ROOT=true` is on THREE PM2 processes (worker + beat + flower), and there is only ONE celery worker.** Pass-10's "both celery workers run `C_FORCE_ROOT=true`" is imprecise — `ecosystem.config.js` defines a single `crm-celery-worker`; `C_FORCE_ROOT` is at lines 58 (worker), 70 (beat), 82 (flower); the Flower `admin:changeme` default is at line 76.
...
> 10. **`fire_webhooks` dispatches via `.delay()`, NOT `on_commit`** (`tickets/services.py:142`) → if the enclosing transaction rolls back, the webhook still fires a payload for a ticket that no longer exists. Also: `crm_webhooks` queue is **consumed but has no producer** (`apps/billing/tasks.py` doesn't exist; ticket webhooks route to `crm_default` via `*`).
...
**Redis:** db3 (cache + `cached_db` sessions, prefix `crm`), db4 (Celery broker; result backend = `django-db`), db5 (Channels layer, prefix `crm:channels`)
**SMTP in-process server:** 2525 (crm-smtp PM2 process, prod only) | **Flower:** 5556 | **TIME_ZONE:** `Asia/Kuala_Lumpur` (⚠ `CELERY_TIMEZONE = "UTC"` — crontab beat entries fire on UTC wall-clock; `USE_TZ=True`)
...
│   ├── checks.py                  # system checks; crm.E001 hard-fails if DEBUG is True (deploy tag)
...
└── .env                           # base.py reads 40 keys (.env.example covers 16; 25 read-but-undocumented; CRM_FLOWER_AUTH documented but NOT read by base.py)
```
...
1. **DEBUG split-default → fixed + hardened.** `main/settings/__init__.py` defaults `DJANGO_DEBUG=False` → an unset var fails safe to `prod.py`; `main/checks.py::crm.E001` hard-fails the deploy check if `DEBUG is True`. CI sets `DJANGO_DEBUG="True"` to keep tests on dev.py.
...
- `main/settings/__init__.py`: `from .base import *`, then `if env.bool("DJANGO_DEBUG", default=False)` → `from .dev import *` (try/except ImportError) **else** `from .prod import *`. Default fails safe to `prod.py`. **`main/checks.py::crm.E001`** hard-fails a deploy check if `DEBUG is True`.
- **Redis:** db3 cache + `cached_db` sessions (`KEY_PREFIX="crm"`); db4 Celery broker (result backend = `django-db`); db5 Channels (`prefix="crm:channels"`).
...
- **`static/js/live-bus.js`** (175 LOC) — global `window.LiveBus`. API: `on/onMany/publish/debounce/rafBatch/isConnected/setChannelState`. Wildcard `"*"` gets all. Cross-tab fan-out via `BroadcastChannel('crm-live')`.
...
### The task — `apps/crm/tasks.py::fire_due_reminders` (Beat `fire-due-reminders` @ 30s → crm_default)
...
- **In-process SMTP server** via `aiosmtpd` (`run_smtp_server`, PM2 `crm-smtp`, default `0.0.0.0:2525`). `handle_RCPT` rejects **550** if no tenant resolves; 25MB cap (**552**); optional STARTTLS + LOGIN/PLAIN auth.
- **IMAP poller** — shared mailbox; UID > watermark. Driven by `fetch_inbound_emails_task` (Beat 60s, `crm_email`). Disabled when `IMAP_HOST` blank. **Never backfills.** **✅ Dedup PER-TENANT** (`filter(tenant, message_id)`, `imap_poller.py:362` — tenant resolved FIRST).
...
**Non-HTTP inbound channel:** `crm-smtp` PM2 process (`0.0.0.0:2525`). **Docs:** `/api/docs/` (Swagger), `/api/schema/` (OpenAPI 3.0). **DRF config:** `DEFAULT_PERMISSION_CLASSES=[IsAuthenticated]`; throttles `[ScopedRateThrottle, APIKeyRateThrottle]`; rates `auth 10/min, api_default 200/min, api_heavy 30/min, webhook 60/min, api_key 1000/hour`; PAGE_SIZE 50.
...
apps.notifications.tasks.send_email_* / send_notification_email → crm_email
apps.inbound_email.tasks.*                        → crm_email
apps.tickets.tasks.send_ticket_*                  → crm_email
apps.api_keys.tasks.send_api_key_*                → crm_email
apps.voip.tasks.*                                 → crm_voip       (⚠ no PM2 worker subscribes)
*                                                 → crm_default
```
...
| `cleanup-stale-calls` | `apps.voip.tasks.cleanup_stale_calls` | 3600s (⚠ piles up — crm_voip unconsumed) |
...
| Name | Purpose |
|------|---------|
| `crm-django` | `gunicorn main.asgi:application -w 2 -k uvicorn.workers.UvicornWorker --bind 0.0.0.0:8001 --timeout 120` (2GB) |
| `crm-celery-worker` | `celery -A main worker -Q crm_default,crm_email,crm_webhooks -c 4 --max-tasks-per-child=200` (2GB) |
| `crm-celery-beat` | `celery -A main beat -l info` |
| `crm-flower` | `celery -A main flower --port=5556 --basic_auth=$CRM_FLOWER_AUTH` |
| `crm-smtp` | `manage.py run_smtp_server` (2525) |

> **⚠ Worker `-Q` = `crm_default,crm_email,crm_webhooks`** — `crm_voip` unconsumed (3 voip tasks + Beat `cleanup-stale-calls` accumulate). `run_ari_listener` **not in PM2**. **Makefile `stop`/`restart` omit `crm-smtp`.**

### `ecosystem.dev.config.js` (dev, venv `env/` → `.venv/`) — 4 processes
- `crm-django` `runserver`. `crm-celery-worker` `-c 2`, **watch** on `apps/*/{tasks,services}.py` + `main/celery.py`. `crm-celery-beat`, `crm-flower`. **No `crm-smtp`.**
...
| `app.js` | **1,288** | Global init: alerts, sidebar (+ close-on-widen ≥992px — Pass 4), notification WS, Toast (now `window.Toast`-published), `CRM.formatDate/…`, badges, live-status pill, **`ReminderAlerts` IIFE** (Feature A) + **`ReminderScheduler` IIFE** (Feature C) |
| `voip-softphone.js` | 710 | SIP.js 0.21.2 + `CallEventConsumer` |
| `custom-select.js` | 371 | `CRMSelect` portal-rendered styled selects |
...
python manage.py run_smtp_server                               # crm-smtp PM2 process
...
- **`CRM_FLOWER_AUTH` is documented but NOT read by `base.py`** — consumed only by `ecosystem.config.js`.
...
19. **VoIP runtime is manual** — `crm_voip` queue unconsumed; `run_ari_listener` not in PM2; `cleanup-stale-calls` Beat messages pile up. `CallEventConsumer` tenant-wide (no per-user scoping).
...
30. **CI EXISTS** (`.github/workflows/ci.yml`, PG16+Redis7). **`requirements.txt` byte-identical to `requirements/base.txt`.** **Logs not rotated.** **`make logs-django` errors.** **`make stop`/`restart` skip `crm-smtp`.** **`pytest-timeout` missing from `dev.txt`.** **`analytics.DashboardView` under-permissioned.**
31. **Security gaps CLOSED:** DEBUG default→False + `main/checks.py::crm.E001` enforcement; messaging tenant-scoping; attachment object-level authz + **authed `/media/`** (`media_views.py`); IMAP per-`(tenant,message_id)` dedup; Stripe replay guard; message-edit fix; HSTS-preload. **`makemigrations --check` clean; 122 migrations** (120 committed incl. `tenants/0011`, + `inbox_hub/0002` [§SLA-Response Fix] + `inbox_hub/0003` [§Triage-Lens Removal backfill] in the working tree).
