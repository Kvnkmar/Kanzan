...
- VERDICT: refuted (unchanged) — After independently reading the cited code in apps/custom_fields/services.py:160-163 and executing actual test cases against the live Django ORM with the Kanzen database, I confirmed that the `Q(visible_to_roles__isnull=True)` filter DOES correctly match CustomFieldDefinition instances with zero visible_to_roles relations. This is standard Django M2M behavior using LEFT OUTER JOIN: when no junctio
...
### [settings-secrets-4] Kanzan_voip queue defined in celery.py but not subscribed by any worker  (OK) [DUP-OF settings-secrets-1]
...
- description: The main/celery.py routes all apps.voip.tasks.* to the 'kanzan_voip' queue, but neither the production worker (ecosystem.config.js line 51) nor the dev worker (ecosystem.dev.config.js line 52) subscribes to kanzan_voip. The worker subscribe lists are only: 'kanzan_default,kanzan_email,kanzan_webhooks'. This means all VoIP tasks — fire_due_reminders (Beat 30s), cleanup_stale_calls (Beat 3600s), sync_call_state, and process_call_recording — silently accumulate in the queue and never execute. The Beat scheduler will continuously try to enqueue cleanup-stale-calls every hour, piling up messages.
- expected: Either (a) add kanzan_voip to the worker's -Q list in ecosystem.config.js and ecosystem.dev.config.js, or (b) remove the kanzan_voip route from celery.py and route all voip tasks to kanzan_default.
- actual: kanzan_voip tasks route to a queue no worker consumes, causing task accumulation and non-execution.
- fix: Option A (preferred if VoIP is used): Update both ecosystem.config.js line 51 and ecosystem.dev.config.js line 52 to: `-Q kanzan_default,kanzan_email,kanzan_webhooks,kanzan_voip`. Option B (if VoIP is not used): Remove the voip route from celery.py or route to kanzan_default.
- VERDICT: confirmed (High) — main/celery.py:27 explicitly routes all apps.voip.tasks.* to "kanzan_voip" queue. ecosystem.config.js:51 and ecosystem.dev.config.js:52 both define the production and dev Celery workers with -Q flag listing only "kanzan_default,kanzan_email,kanzan_webhooks" — kanzan_voip is absent from both. main/settings/base.py:318-321 confirms cleanup_stale_calls is in the Beat schedule at 3600s (hourly). apps/
