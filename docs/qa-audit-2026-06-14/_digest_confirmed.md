...
### [settings-secrets-6] Kanzan_webhooks queue routed but empty (no actual billing task consumers)
cat=Reliability | conf=medium | verdict=unverified | merged=[]
LOC: main/celery.py:21, apps/billing/tasks.py (does not exist)
DESC: The main/celery.py routes 'apps.billing.tasks.*' to 'kanzan_webhooks' queue, which IS consumed by the worker. However, /home/kavin/Kanzen/apps/billing/tasks.py does not exist. This queue is technically operational but carries no tasks (since there are no billing tasks defined). The worker wastes a queue subscription on an empty route, and any billing tasks defined in the future would silently be dropped unless the developer remembers this route and creates the file.
REPRO: 1. Check ecosystem.config.js: worker subscribes to kanzan_webhooks. 2. Check main/celery.py: route points to apps.billing.tasks.*. 3. Check apps/billing/: no tasks.py file exists. 4. If a developer adds a billing task without remembering this route, the task goes to kanzan_webhooks but kanzan_webhooks has no handler logic.
EXPECTED: Either (a) remove the unused kanzan_webhooks route and let billing tasks default to kanzan_default, or (b) create a minimal apps/billing/tasks.py with a comment explaining the route, or (c) document this footgun clearly.
ACTUAL: kanzan_webhooks queue is routed but no tasks use it, creating a silent bug-prone configuration.
...
### [settings-secrets-8] Makefile 'stop' and 'restart' targets omit kanzan-smtp process
cat=Reliability | conf=medium | verdict=unverified | merged=[]
LOC: Makefile:56, 60
DESC: The Makefile 'stop' and 'restart' targets explicitly list the processes to stop/restart but omit 'kanzan-smtp'. Line 56: `pm2 stop kanzan-django kanzan-celery-worker kanzan-celery-beat kanzan-flower` (no kanzan-smtp). This means calling 'make stop' or 'make restart' leaves the SMTP server running in production, potentially accepting stale mail or serving outdated code. The SMTP server has no auto-reload mechanism and would continue running indefinitely.
REPRO: 1. Run `make stop` in production. 2. Observe that kanzan-smtp process is still running (check `pm2 list`). 3. New deployments using `make restart` won't restart SMTP, leading to old SMTP code running.
EXPECTED: Both 'stop' and 'restart' targets should include 'kanzan-smtp' in the pm2 command list.
ACTUAL: kanzan-smtp is omitted from 'stop' and 'restart' commands, causing it to keep running.
FIX: Update Makefile lines 56 and 60 to include kanzan-smtp: `pm2 stop kanzan-django kanzan-celery-worker kanzan-celery-beat kanzan-flower kanzan-smtp` and `pm2 restart kanzan-django kanzan-celery-worker kanzan-celery-beat kanzan-flower kanzan-smtp`
