/**
 * CRM - PM2 Process Manager Configuration
 *
 * Port allocation (avoids conflicts with Tempest on port 8000):
 *   ASGI server: Unix socket /tmp/crm.sock (Nginx proxies 8001 here)
 *   Redis: db3 (cache), db4 (Celery broker), db5 (Channels layer)
 *   Celery queues: crm_default, crm_email, crm_webhooks
...
module.exports = {
  apps: [
...

    {
      ...
      args: "-A main worker -Q crm_default,crm_email,crm_webhooks -c 4 -l info --pool prefork -n crm-worker@%h --max-tasks-per-child=200",
      interpreter: "none",
      ...COMMON_CONFIG,
      max_memory_restart: "2G",
      kill_timeout: 15000,
